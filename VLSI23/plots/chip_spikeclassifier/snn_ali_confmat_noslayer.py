...
# Texture 180mVpp
#CUDA_VISIBLE_DEVICES=3 python snn_ali_confmat_noslayer.py --datasetPath /users/micas/malea/Documents/ArduinoRead_2/ArduinoRead/out/N_3b_allchannels_18mvpp_250mvofs_ALLclass_auto_nospkfilter_sorted/ --epochs 150 --hidden_size 512 --num_classes 6 --batch_size 12 --tmax_ms 400 --Ts 1 --kfolds 6 --cache yes 

# Texture 7.2mVpp
#CUDA_VISIBLE_DEVICES=0 python snn_ali_confmat_noslayer.py --datasetPath /users/micas/malea/Documents/ArduinoRead_2/ArduinoRead/out/N_4b_allchannels_ALLCLASS_auto_nospkfilter_cleaned_12x16_7p20mvpp_250mvofs/ --epochs 150 --hidden_size 256 --num_classes 6 --batch_size 32 --tmax_ms 400 --Ts 10 --kfolds 6 --cache yes --save_bestmodel yes

# Texture 9mVpp
#CUDA_VISIBLE_DEVICES=0 python snn_ali_confmat_noslayer.py --datasetPath /users/micas/malea/Documents/ArduinoRead_2/ArduinoRead/out/N_4b_allchannels_ALLCLASS_auto_nospkfilter_cleaned_12x16_9mvpp_250mvofs/ --epochs 150 --hidden_size 256 --num_classes 6 --batch_size 32 --tmax_ms 400 --Ts 10 --kfolds 6 --cache yes --save_bestmodel yes

# Model texture 7.2mVpp
#CUDA_VISIBLE_DEVICES=0 python snn_ali_confmat_noslayer.py --datasetPath /users/micas/malea/Documents/all_scripts-slayer/global_outfile/spikegen/ALL_N_lcadc-0-uV-v2-0-uV-0-uV-0-pct-2-mr_cnt-192-multitrial-Kylberg_filt_6_scan_actualdimscale_chip--1to40-1to40_sorted/ --epochs 150 --hidden_size 256 --num_classes 6 --batch_size 32 --tmax_ms 400 --Ts 10 --kfolds 6 --cache yes --save_bestmodel yes

# Vibration
#CUDA_VISIBLE_DEVICES=0 python snn_ali_confmat_noslayer.py --datasetPath /users/micas/malea/Documents/ArduinoRead_2/ArduinoRead/out/N_3b_allchannels_ALLCLASS_auto_nospkfilter_cleaned_12x16_vibration/ --epochs 150 --hidden_size 128 --num_classes 6 --batch_size 32 --tmax_ms 400 --Ts 10 --kfolds 6 --cache yes --save_bestmodel yes


# Texture 180mVpp (new)
#CUDA_VISIBLE_DEVICES=3 python snn_ali_confmat_noslayer.py --datasetPath /users/micas/malea/Documents/ArduinoRead_2/ArduinoRead/out/N_4b_allchannels_ALLCLASS_auto_nospkfilter_cleaned_12x16_180mvpp_250mvofs_onedac/ --epochs 150 --hidden_size 256 --num_classes 6 --batch_size 32 --tmax_ms 400 --Ts 10 --kfolds 6 --cache yes --save_bestmodel yes
...
extraPath =  'epochs-' + str(args.epochs) + '-input_size-' + str(ch_size) + '-' + str(x_size) + '-' + str(y_size) + '-hidden_size-' + str(args.hidden_size) + '-batch_size-' + str(args.batch_size) + '-lr-' + str(learning_rate) + '-tmax_ms-' + str(args.tmax_ms) + '-Ts-' + str(args.Ts) + "-kfolds-" + str(args.kfolds) + datasetPath
    ...
    for e in range(epochs):
        ...
        if args.save_bestmodel:
            if train_acc/total >= np.max(acc_hist_train):
                ...
                if args.retrain:
                    torch.save(model.state_dict(), savePath + '/chip-classifier-retrain' + str(fold) + '-best-.pt')
                else:
...
def test(model, testloader, batch_size, acc_hist_test, fold):
    ...
    if args.save_bestmodel:
        if test_acc/total >= np.max(acc_hist_test):
            ...
            if args.retrain:
                torch.save(model.state_dict(), savePath + '/chip-classifier-test-retrain' + str(fold) + '-best-.pt')
            else:
...
#netParams['training']['path']['test'] = sampleFile_test
    ...
    for fold, (train_ids, test_ids) in enumerate(kfold.split(dataset)):
      ...
      ## Save model
      if args.retrain:
          torch.save(model.state_dict(), savePath + '/chip-classifier-retrain-' + str(fold) + '.pt')
      else:
      ...
      if args.save_bestmodel:
          if args.retrain:
              model_best.load_state_dict(torch.load( savePath + '/chip-classifier-retrain-' + str(fold) + '-best.pt'))
          else:
