...
#CUDA_VISIBLE_DEVICES=0 python snn_ali_confmat_noslayer_pearsonsvmonly_nodataloader.py --datasetPath /users/micas/malea/Documents/ArduinoRead_2/ArduinoRead/out/N_3b_allchannels_ALLCLASS_auto_nospkfilter_cleaned_12x16_180mvpp_250mvofs/ --epochs 10 --hidden_size 512 --num_classes 6 --batch_size 12 --tmax_ms 400 --Ts 1 --kfolds 6 --cache yes
...
"""
Train SVM Classifier on the Pearson Correlation Coefficient Matrix Extracted from the Filtered Output Spikes of the Hidden Layer
...
if args.retrainPath:
    ...
    #print(modelname[0])
    retrainPath = '-retrainPath-' + modelname[0]
else:
    retrainPath = ''
...
extraPath =  'pearsonsvmonly-epochs-' + str(args.epochs) + '-input_size-' + str(ch_size) + '-' + str(x_size) + '-' + str(y_size) + '-hidden_size-' + str(args.hidden_size) + '-batch_size-' + str(args.batch_size) + '-svm_window_size-' + str(window_size) + '-lr-' + str(learning_rate) + '-tmax_ms-' + str(args.tmax_ms) + '-Ts-' + str(args.Ts) + "-kfolds-" + str(args.kfolds)  + retrainPath + datasetPath
...
def train_test(model, train_loader, test_loader, optimizer, epochs, batch_size, grid_search_linear, grid_search_rbf):
    ...
    for e in range(epochs):
        ...
        acc_hist_train_svm_rbf.append(correct_rbf/total)           
...
#netParams['training']['path']['test'] = sampleFile_test
    ...
    # save fold results to txt files
