...
#CUDA_VISIBLE_DEVICES=3 python snn_ali_confmat_noslayer_svm.py --datasetPath /users/micas/malea/Documents/ArduinoRead_2/ArduinoRead/out/N_3b_allchannels_18mvpp_250mvofs_ALLclass_auto_nospkfilter_sorted/ --epochs 300 --hidden_size 512 --num_classes 6 --batch_size 12 --tmax_ms 400 --Ts 1 --kfolds 2 --cache yes 
#CUDA_VISIBLE_DEVICES=3 python snn_ali_confmat_noslayer_svm.py --datasetPath /users/micas/malea/Documents/ArduinoRead_2/ArduinoRead/out/N_3b_allchannels_ALLCLASS_auto_nospkfilter_cleaned_12x16_180mvpp_250mvofs/ --epochs 10 --hidden_size 512 --num_classes 6 --batch_size 12 --tmax_ms 400 --Ts 1 --kfolds 6 --cache yes 
...
extraPath =  'epochs-' + str(args.epochs) + '-input_size-' + str(ch_size) + '-' + str(x_size) + '-' + str(y_size) + '-hidden_size-' + str(args.hidden_size) + '-batch_size-' + str(args.batch_size) + '-svm_window_size-' + str(window_size) + '-lr-' + str(learning_rate) + '-tmax_ms-' + str(args.tmax_ms) + '-Ts-' + str(args.Ts) + "-kfolds-" + str(args.kfolds) 
...
def train(model, train_loader, test_loader, optimizer, epochs, batch_size, acc_hist_test):
    ...
    hidden_spike_filt_max = []
    for e in range(epochs):
        train_acc = 0
        train_loss_sum = 0
        total = 0
        hidden_spike_filt_append = []
        labels_append = []
        ...
        for i, (images, labels) in enumerate(train_loader):
            ...
            hidden_spike_filt_append.append(hidden_spike_filt)
            labels_append.append(labels)
            #labels_append = torch.cat((labels_append,labels),dim=0)
        ...
        print("Fold: " + str(fold) + " Epoch: " + str(e) + " Loss: " + str(train_loss_sum.item()/total) + " Train Accuracy: " + str(train_acc/total))

        # Test per epoch -> comment if too slow
        acc_hist_test = test(model, test_loader, batch_size, acc_hist_test)

        hidden_spike_filt_append = torch.cat(hidden_spike_filt_append,dim=0)
        labels_append = torch.cat(labels_append,dim=0)

        #print(type(hidden_spike_filt_append))
        #print(hidden_spike_filt_append.shape)

        if train_acc/total >= np.max(acc_hist_train):
            #print("max train acc")
            #print(train_acc/total)
            #print(np.max(acc_hist_train))
            hidden_spike_filt_max = torch.tensor(hidden_spike_filt_append)
            print(hidden_spike_filt_max)
    ...
    #hidden_spike_filt_np = hidden_spike_filt_max.numpy()
    hidden_spike_filt_np = hidden_spike_filt_max.detach().numpy()

    #for i in range(hidden_spike_filt_np.shape[0]):
    ...
    #print(np.shape(hidden_spike_filt_sc))
    #print(np.shape(labels_append))

    # Define the hyperparameter grid for the linear SVC classifier
    param_grid_linear = {'C': [0.1, 1, 10, 100]}

    # Create a linear SVC classifier
    clf_linear = SVC(kernel='linear')

    # Create a GridSearchCV object to tune the hyperparameters of the linear SVC classifier
    grid_search_linear = GridSearchCV(clf_linear, param_grid_linear, cv=3)

    # Train the classifier using hidden_spike_filt and labels
    grid_search_linear.fit(hidden_spike_filt_sc, labels_append)

    # Print the best hyperparameters and the best score
    print('Best hyperparameters for the linear SVC classifier:', grid_search_linear.best_params_)
    print('Best score for the linear SVC classifier:', grid_search_linear.best_score_)

    # get SVM accuracy using the best trained model (with normal softmax)
    acc_hist_test_svm_max = test_svm(model, testloader, batch_size, grid_search_linear)

    return acc_hist_test, acc_hist_test_svm_max, grid_search_linear
...
def test(model, testloader, batch_size, acc_hist_test):
    ...
    for i, (images, labels) in enumerate(testloader):
      ...
      predictions, test_loss, nbr_events_avrg, _ = model(images, labels, hidden_mem, hidden_spike, output_mem, output_spike)
      predicted = predictions
      ...
      if args.save_bestmodel:
        ...
        predictions, _, _, _ = model_best(images, labels, hidden_mem, hidden_spike, output_mem, output_spike)
        predicted = predictions
...
def test_svm(model, testloader, batch_size, grid_search_linear):
    correct = 0
    total = 0
    correct_best = 0
    #test_acc = 0
    ...
    scaler = StandardScaler()
    # batch_size x hidden_size x timewindows
    hidden_spike_filt_np = hidden_spike_filt_append.detach().numpy()
    ...
    # Train the classifier using hidden_spike_filt and labels
    labels_predicted = grid_search_linear.predict(hidden_spike_filt_sc)
    labels_predicted = torch.tensor(labels_predicted)
    print(labels_append)
    print(labels_predicted)
    print(labels_predicted == labels_append)
    correct_svm = (labels_predicted == labels_append).sum() 
    acc_test_svm = correct_svm.data.cpu().numpy()/labels_append.shape[0]
    print("SVM Test Accuracy @ best epoch: " + str(acc_test_svm))
...
#netParams['training']['path']['test'] = sampleFile_test
    ...
    for fold, (train_ids, test_ids) in enumerate(kfold.split(dataset)):
      ...
      # Training
      acc_hist_test_, acc_hist_test_svm_max, grid_search_linear = train(model, trainloader, testloader, optimizer, num_epochs, batch_size, acc_hist_test) 
      ...
      # Evaluationfor this fold (final accuracy after epochs)
      for i, (images, labels) in enumerate(testloader):
        ...
        #gather all filtered spike  & labels across all samples of the testloader -> SVM
        hidden_spike_filt_append.append(hidden_spike_filt)
        labels_append.append(labels)
      ...
      #initialize scaler to remove mean
      scaler = StandardScaler()
      # batch_size x hidden_size x timewindows
      hidden_spike_filt_np = hidden_spike_filt_append.detach().numpy()
      ...
      print(labels_predicted)
      correct_svm = (labels_predicted == labels_append).sum() 
      svm_test_acc = correct_svm.data.cpu().numpy()/labels_append.shape[0]
      print("SVM Test Accuracy after training: " + str(svm_test_acc))
      ...
      print('--------------------------------')
      print('SVM Accuracy for fold %d: %f %%' % (fold, 100.0 * svm_test_acc))
      print('--------------------------------')
      print('--> At epoch = %d, Max: %f %%' % (epoch_max, 100.0 * acc_hist_test_max))     
      print('--------------------------------')
      print('--> At epoch = %d, SVM Max: %f %%' % (epoch_max, 100.0 * acc_hist_test_svm_max))       
      results[fold] = 100.0 * (test_acc_)
      results_svm[fold] = 100.0 * svm_test_acc
      results_max[fold] =  100.0 * (acc_hist_test_max)
      results_svm_max[fold] = 100.0 * acc_hist_test_svm_max
    ...
    print(f'K-FOLD CROSS VALIDATION RESULTS FOR {k_folds} FOLDS - SVM')
    print('--------------------------------')
    sum = 0.0
    ...
    # Print fold results
    print(f'MAX:   K-FOLD CROSS VALIDATION RESULTS FOR {k_folds} FOLDS - SVM')
    print('--------------------------------')
    sum = 0.0
    for key, value in results_svm_max.items():
      print(f'Fold {key}: {value} %')
      sum += value
    print(f'MAX Average: {sum/len(results.items())} %')
