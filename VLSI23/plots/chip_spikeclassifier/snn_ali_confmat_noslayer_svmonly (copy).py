...
#CUDA_VISIBLE_DEVICES=0 python snn_ali_confmat_noslayer_svmonly.py --datasetPath /users/micas/malea/Documents/ArduinoRead_2/ArduinoRead/out/N_3b_allchannels_ALLCLASS_auto_nospkfilter_cleaned_12x16_180mvpp_250mvofs/ --epochs 150 --hidden_size 512 --num_classes 6 --batch_size 12 --tmax_ms 400 --Ts 1 --kfolds 6 --cache yes
...
def train_test(model, train_loader, test_loader, optimizer, epochs, batch_size):
    ...
    grid_search_linear = GridSearchCV(linear_svm, parameters_linear, cv=1)

    # Initialize the grid search object for the SVM classifier with an RBF kernel
    grid_search_rbf = GridSearchCV(rbf_svm, parameters_rbf, cv=1)
    ...
    for e in range(epochs):
        ...
        # Print the best hyperparameters and the best score
        print("Fold: " + str(fold) + " Epoch: " + str(e) + "  Best hyperparameters for the linear SVC classifier:", grid_search_linear.best_params_)
        print("Fold: " + str(fold) + " Epoch: " + str(e) + "  Best test score for the linear SVC classifier:", grid_search_linear.best_score_)
 
        # Print the best hyperparameters and the best score
        print("Fold: " + str(fold) + " Epoch: " + str(e) + "  Best hyperparameters for the RBF SVC classifier:", grid_search_rbf.best_params_)
        print("Fold: " + str(fold) + " Epoch: " + str(e) + "  Best test score for the RBF SVC classifier:", grid_search_linear.best_score_)
...
#netParams['training']['path']['test'] = sampleFile_test
    ...
    for fold, (train_ids, test_ids) in enumerate(kfold.split(dataset)):
      ...
      acc_hist_train_svm_linear, acc_hist_train_svm_rbf, acc_hist_test_svm_linear, acc_hist_test_svm_rbf = train_test(model, trainloader, testloader, optimizer, num_epochs, batch_size) 
