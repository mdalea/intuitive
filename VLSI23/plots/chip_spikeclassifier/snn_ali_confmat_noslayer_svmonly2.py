...
#CUDA_VISIBLE_DEVICES=0 python snn_ali_confmat_noslayer_svmonly2.py --datasetPath /users/micas/malea/Documents/ArduinoRead_2/ArduinoRead/out/N_3b_allchannels_18mvpp_250mvofs_ALLclass_auto_nospkfilter_sorted/ --epochs 10 --hidden_size 512 --num_classes 6 --batch_size 12 --tmax_ms 400 --Ts 1 --kfolds 6 --cache yes

#CUDA_VISIBLE_DEVICES=0 python snn_ali_confmat_noslayer_svmonly2.py --datasetPath /users/micas/malea/Documents/ArduinoRead_2/ArduinoRead/out/N_3b_allchannels_ALLCLASS_auto_nospkfilter_cleaned_12x16_180mvpp_250mvofs/ --epochs 10 --hidden_size 512 --num_classes 6 --batch_size 12 --tmax_ms 400 --Ts 1 --kfolds 6 --cache yes
...
# Dataset definition
class SpkDataset_toFrameTimeBins_Filt(Dataset):
    def __init__(self, datasetPath, sampleFile, samplingTime, window_size):
        self.path = datasetPath 
        self.samples = np.loadtxt(sampleFile).astype('int')
        self.samplingTime = float(samplingTime)
    ...
    def __getitem__(self, index):
        ...
        frames = frame_transform(events).astype(np.float32)
        #print(np.shape(frames))
        frames = frames.reshape(-1, ch_size*x_size*y_size)
        ...
        # Init an untrained neural network
        ...
        return hidden_spike_filt_sc,classLabel
...
#netParams['training']['path']['test'] = sampleFile_test
    ...
    trainingSet = SpkDataset_toFrameTimeBins_Filt(datasetPath =args.datasetPath, 
    ...
    #                            samplingTime=netParams['simulation']['Ts'],
    #                            sampleLength=netParams['simulation']['tSample'])
    testingSet = SpkDataset_toFrameTimeBins_Filt(datasetPath  =args.datasetPath, 
    ...
    parameters_linear = {'C': [0.001, 0.1, 1, 10, 100]}

    # Set the hyperparameters for the SVM classifier with an RBF kernel
    parameters_rbf = {'C': [0.001, 0.1, 1, 10, 100], 'gamma': [0.1, 1, 10, 100]}
    ...
    for fold, (train_ids, test_ids) in enumerate(kfold.split(dataset)):
      ...
      for e in range(num_epochs):
        ...
        grid_search_linear.fit(trainloader)

        # Train the RBF classifier 
        grid_search_rbf.fit(trainloader)

        score_linear = grid_search_linear.score(testloader)

        # Train the RBF classifier 
        score_rbf = grid_search_rbf.score(testloader)
        ...
        acc_hist_test_svm_linear.append(score_linear)
        acc_hist_test_svm_rbf.append(score_rbf)    
        ...
        print("Fold: " + str(fold) + " Epoch: " + str(e) + "  Best train score for the linear SVC classifier:", grid_search_linear.best_score_)

        # Print the best hyperparameters and the best score
        print("Fold: " + str(fold) + " Epoch: " + str(e) + "  Best hyperparameters for the RBF SVC classifier:", grid_search_rbf.best_params_)
        print("Fold: " + str(fold) + " Epoch: " + str(e) + "  Best train score for the RBF SVC classifier:", grid_search_linear.best_score_)
