...
#CUDA_VISIBLE_DEVICES=0 python snn_ali_confmat_noslayer_svmonly_nodataloader.py --datasetPath /users/micas/malea/Documents/ArduinoRead_2/ArduinoRead/out/N_3b_allchannels_ALLCLASS_auto_nospkfilter_cleaned_12x16_180mvpp_250mvofs/ --epochs 10 --hidden_size 512 --num_classes 6 --batch_size 12 --tmax_ms 400 --Ts 1 --kfolds 6 --cache yes

#CUDA_VISIBLE_DEVICES=0 python snn_ali_confmat_noslayer_svmonly_nodataloader.py --datasetPath /users/micas/malea/Documents/ArduinoRead_2/ArduinoRead/out/N_3b_allchannels_18mvpp_250mvofs_ALLclass_auto_nospkfilter_sorted/ --epochs 10 --hidden_size 512 --num_classes 6 --batch_size 12 --tmax_ms 400 --Ts 1 --kfolds 6 --cache yes

#CUDA_VISIBLE_DEVICES=0 python snn_ali_confmat_noslayer_svmonly_nodataloader.py --datasetPath /users/micas/malea/Documents/ArduinoRead_2/ArduinoRead/out/N_3b_allchannels_ALLCLASS_auto_nospkfilter_cleaned_12x16_180mvpp_250mvofs/ --epochs 10 --hidden_size 512 --num_classes 6 --batch_size 32 --tmax_ms 400 --Ts 10 --kfolds 6 --cache yes --svm_window_size 5 --retrainPath /users/micas/malea/Documents/chip_spikeclassifier/TrainedALL_epochs-50-input_size-2-16-12-hidden_size-512-batch_size-32-lr-0.001-tmax_ms-400.0-Ts-10.0-kfolds-6/chip-classifier-0-best.pt

#CUDA_VISIBLE_DEVICES=0 python snn_ali_confmat_noslayer_svmonly_nodataloader.py --datasetPath /users/micas/malea/Documents/ArduinoRead_2/ArduinoRead/out/N_4b_allchannels_ALLCLASS_auto_nospkfilter_cleaned_12x16_7p20mvpp_250mvofs/ --epochs 5 --hidden_size 256 --num_classes 6 --batch_size 32 --tmax_ms 400 --Ts 10 --kfolds 6 --cache yes --svm_window_size 40 --scaler yes --retrainPath /users/micas/malea/Documents/chip_spikeclassifier/TrainedALL_epochs-50-input_size-2-16-12-hidden_size-256-batch_size-32-lr-0.001-tmax_ms-400.0-Ts-10.0-kfolds-6-datasetPath-N_4b_allchannels_ALLCLASS_auto_nospkfilter_cleaned_12x16_7p20mvpp_250mvofs/chip-classifier-test-0-best.pt
...
parser.add_argument("--retrainPath1",type=str,help="Re-train existing network. Enter model path .pt",required=False)
parser.add_argument("--retrainPath2",type=str,help="Re-train existing network. Enter model path .pt",required=False)
...
if args.retrainPath1:
    tokens_slash = args.retrainPath1.split('/')
    #print(tokens_slash[-1])
    modelname = tokens_slash[-1].split('.')
    #print(modelname[0])
    retrainPath1 = '-retrainPath1-' + modelname[0]
else:
    retrainPath1 = ''

if args.retrainPath2:
    tokens_slash = args.retrainPath2.split('/')
    #print(tokens_slash[-1])
    modelname = tokens_slash[-1].split('.')
    #print(modelname[0])
    retrainPath2 = '-retrainPath2-' + modelname[0]
else:
    retrainPath2 = ''
...
extraPath =  'svmonly_concat-epochs-' + str(args.epochs) + '-input_size-' + str(ch_size) + '-' + str(x_size) + '-' + str(y_size) + '-hidden_size-' + str(args.hidden_size) + '-batch_size-' + str(args.batch_size) + '-svm_window_size-' + str(window_size) + '-lr-' + str(learning_rate) + '-tmax_ms-' + str(args.tmax_ms) + '-Ts-' + str(args.Ts) + "-kfolds-" + str(args.kfolds) + scaler + retrainPath1 + retrainPath2 + datasetPath
...
def train_test(model1, model2, train_loader, test_loader,  epochs, batch_size, grid_search_linear, grid_search_rbf):
    ...
    for e in range(epochs):
        ...
        hidden_mem1 =  torch.zeros(model1.hidden_size_1) # random init
        output_mem1 =  torch.zeros(model1.output_size) #random init
        hidden_spike1 = torch.zeros(model1.hidden_size_1)
        output_spike1 = torch.zeros(model1.output_size)

        hidden_mem2 =  torch.zeros(model2.hidden_size_1) # random init
        output_mem2 =  torch.zeros(model2.output_size) #random init
        hidden_spike2 = torch.zeros(model2.hidden_size_1)
        output_spike2 = torch.zeros(model2.output_size)
            
        #print(images.shape)

        hidden_spike_filt1 = model1(images, labels, hidden_mem1, hidden_spike1, output_mem1, output_spike1)
        hidden_spike_filt2 = model2(images, labels, hidden_mem2, hidden_spike2, output_mem2, output_spike2)

        # batch_size x hidden_size x timewindows
        hidden_spike_filt_np1 = hidden_spike_filt1.detach().numpy()
        hidden_spike_filt_np2 = hidden_spike_filt2.detach().numpy()
        hidden_spike_filt_np = np.concatenate(hidden_spike_filt1, hidden_spike_filt2, axis=0)
...
#netParams['training']['path']['test'] = sampleFile_test
    ...
    for fold, (train_ids, test_ids) in enumerate(kfold.split(dataset)):
      ...
      model1 = mini_eMLP(ch_size, x_size, y_size, hidden_size, num_classes, window_size=window_size)
      model2 = mini_eMLP(ch_size, x_size, y_size, hidden_size, num_classes, window_size=window_size)
      ...
      if args.retrainPath1:
          model1.load_state_dict(torch.load(args.retrainPath1))     

      if args.retrainPath2:
          model2.load_state_dict(torch.load(args.retrainPath2))     




      #base_params = [model.i2h.weight, model.i2h.bias, model.h2o.weight, model.h2o.bias]
      #base_params1 = [model1.i2h.weight, model1.i2h.bias, model1.h2o.weight, model1.h2o.bias, model1.h2h.weight, model1.h2h.bias]
      #base_params2 = [model.i2h.weight, model.i2h.bias, model.h2o.weight, model.h2o.bias, model2.h2h.weight, model2.h2h.bias]
      ...
      acc_hist_train_svm_linear, acc_hist_train_svm_rbf, acc_hist_test_svm_linear, acc_hist_test_svm_rbf = train_test(model1, model2, trainloader, testloader, num_epochs, batch_size, grid_search_linear, grid_search_rbf) 
