% inspired from Praneeth Namburi's code
...
for i=1:length(tex_array)
    ...
    tbch = tbch_array(i)
                ...
                cmd1 = ['"C:\Users\iclab\Documents\ArduinoRead_2\arduino-cli_0.29.0_Windows_64bit\arduino-cli" compile --fqbn arduino:avr:mega "C:\Users\iclab\Documents\ArduinoRead_2\ArduinoRead\matlab_code_to_generate_vdac_values\ino_files/',vdac_cfg,'\ALLCH0-class-',num2str(k),'-amp-',num2str(amp),'\ALLCH0-class-',num2str(k),'-amp-',num2str(amp),'.ino"']
                %cmd1 = ['"C:\Users\iclab\Documents\ArduinoRead_2\arduino-cli_0.29.0_Windows_64bit\arduino-cli" compile --fqbn arduino:avr:mega "C:\Users\iclab\Documents\ArduinoRead_2\ArduinoRead\matlab_code_to_generate_vdac_values\ino_files/ALLCH0-class-',num2str(k),'-amp-',num2str(trial),'\ALLCH0-class-',num2str(k),'-amp-',num2str(trial),'.ino"']

                system(cmd1)
                cmd2 = ['"C:\Users\iclab\Documents\ArduinoRead_2\arduino-cli_0.29.0_Windows_64bit\arduino-cli" upload  -p COM4 --fqbn arduino:avr:mega "C:\Users\iclab\Documents\ArduinoRead_2\ArduinoRead\matlab_code_to_generate_vdac_values\ino_files/',vdac_cfg,'\ALLCH0-class-',num2str(k),'-amp-',num2str(amp),'\ALLCH0-class-',num2str(k),'-amp-',num2str(amp),'.ino"']
                %cmd2 = ['"C:\Users\iclab\Documents\ArduinoRead_2\arduino-cli_0.29.0_Windows_64bit\arduino-cli" upload  -p COM4 --fqbn arduino:avr:mega "C:\Users\iclab\Documents\ArduinoRead_2\ArduinoRead\matlab_code_to_generate_vdac_values\ino_files\ALLCH0-class-',num2str(k),'-amp-',num2str(trial),'\ALLCH0-class-',num2str(k),'-amp-',num2str(trial),'.ino"']
                ...
                %name = ['N_3b_allchannels_180mvpp_250mvofs_cl',num2str(k),'_auto'];
                arduinoObj.UserData = struct("Count",1, "TD_x",[],"TD_y",[],"TD_p",[],"TD_ts",[], "name", name, "trial", trial, "trialcnt", trialcnt, "amp", amp, "tbch", tbch, "verbose", verbose);
...
trialcnt = src.UserData.trialcnt;
amp = src.UserData.amp;
...
if spk_data(1) == 'S'
    ...
    if src.UserData.Count > 1000  %2000 %8000
        %spks_complete = 1; 
        f0 = fopen('spks_complete.txt','w+');
        fprintf(f0,'%i','1');
        fclose(f0);
...
%         %Encode_Ndataset([folderPath,'/',num2str(1)],TD);
        ...
        Encode_Ndataset_longTs([folderPath,'\out\',name,'\',num2str(trialcnt*(amp-1)+trial),'-',num2str(tbch)],TD);
        TD=dataset_sort_timestep_arduino_12x16(folderPath,['\out\',name,'\',num2str(trialcnt*(amp-1)+trial),'-',num2str(tbch)]);    
