...
inst_id = sys.argv[1] + '_ofs_sweep_'
...
end_vofs = float(sys.argv[4]) #200e-3
step_size = float(sys.argv[5]) #5e-3
...
for run_id in range(0,int(sys.argv[2])): # perform N runs for averaging
    ...
    gain = []
    gain_db = []
    for vofs in range(int(start_vofs * 1000), int(end_vofs * 1000) + 1, int(step_size * 1000)):
        vofs = vofs / 1000.0
    ...
    rows = zip(vofs_arr, vout_pp_arr, vin_pp_arr, gain, gain_db)

    # Specify the CSV file path
    csv_file_path = 'output\ID_' + inst_id + '_gain_vs_vofsin_sweep' + '-' + str(run_id) + '.csv'

    # Write the data to the CSV file
    with open(csv_file_path, 'w', newline='') as csvfile:
        csv_writer = csv.writer(csvfile)
        csv_writer.writerow(['vofs',  'vout_pp' , 'vin_pp', 'gain', 'gain_db'])  # Write the header row
