...
%% Combined plots for paper (modified flicker noise to integrated power)
...
wnoise_arr_ext = [1e-9 wnoise_arr]; % replace 0 with small value
fnoise_arr_ext = [1e-9 fnoise_arr].*log(10e3/1);
fnoise_arr_str_1 = sprintf('%.1e', fnoise_arr_ext(1))
fnoise_arr_str_2 = sprintf('%.1e', fnoise_arr_ext(2))
fnoise_arr_str_3 = sprintf('%.1e', fnoise_arr_ext(3))
fnoise_arr_str_4 = sprintf('%.1e', fnoise_arr_ext(4))

fnoise_arr_ext(1) = str2double(fnoise_arr_str_1)
fnoise_arr_ext(2) = str2double(fnoise_arr_str_2)
fnoise_arr_ext(3) = str2double(fnoise_arr_str_3)
fnoise_arr_ext(4) = str2double(fnoise_arr_str_4)
...
h(2) = errorbar(fnoise_arr_ext, results_linear_maxfold_fnoise_arr_trainonclean(1:4)./results_linear_maxfold_fnoise_arr(1), [std(results_linear_maxfold_fnoise_trainonclean(1,:))./results_linear_maxfold_fnoise_arr(1) std(results_linear_maxfold_fnoise_trainonclean(2,:))./results_linear_maxfold_fnoise_arr(1) std(results_linear_maxfold_fnoise_trainonclean(3,:))./results_linear_maxfold_fnoise_arr(1) std(results_linear_maxfold_fnoise_trainonclean(4,:))./results_linear_maxfold_fnoise_arr(1)],'--','Color',[0.4940 0.1840 0.5560],'LineWidth',3); 
xlim([0 max(fnoise_arr_ext)])
xlab_fnoise= xlabel('AFE flicker noise power(V^{2})')
%ylab_fnoise= ylabel(['Normalized',newline,'Classification',newline,'Accuracy',newline,' (10 k-folds)'])
hold off;
xticks = unique([0 fnoise_arr_ext(2:4)]);
set(gca, 'YGrid', 'on');
...
% perform t-test to evaluate statistical significance away from noiseless
% performance
t_wnoise = (results_linear_maxfold_wnoise_arr(2:4) - results_linear_maxfold_wnoise_arr(1)) ./ sqrt(std(results_linear_maxfold_wnoise(1,:))./kfolds + std(results_linear_maxfold_wnoise(2:4,:))./kfolds)
